...
const Login = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    await (dispatch as unknown as (fn: ReturnType<typeof login>) => Promise<void>)(login(email, password));
      ...
      <div className="bg-mongo-card border border-mongo-border rounded-xl p-8">
        <div className="mb-6">
          ...
          <p className="text-mongo-muted mt-1 text-sm">
            Welcome back! Sign in to continue your revision.
          </p>
        </div>
        ...
        <p className="text-center text-sm text-mongo-muted mt-6">
          ...
          <Link to="/register" className="text-mongo-green hover:underline">
            Sign Up
          </Link>
        </p>
      </div>
